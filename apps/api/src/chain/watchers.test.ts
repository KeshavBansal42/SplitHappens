...
const hoisted = vi.hoisted(() => {
  // In-memory chain stub created inline (no module imports allowed here).
  const state = {
...
const ESCROW = "0x2222222222222222222222222222222222222222"; // matches setupEnv
...
describe("confirmTick", () => {
  ...
  it("leaves the row pending when the receipt is not mined yet", async () => {
    seedSplit();
    seedPendingParticipant();
    // No receipt in the stub -> the chain call throws -> stay pending.
...
/** Calldata for deposit(uint256,uint256); selector 0xe2bbb158 (from viem). */
