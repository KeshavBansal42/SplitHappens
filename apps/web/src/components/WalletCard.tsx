...
export function WalletCard() {
  ...
  if (!authenticated) {
    return (
      <section className="card">
        <h2>Wallet</h2>
        <p>Log in with email to get your embedded wallet.</p>
        <LoginButton />
      </section>
    );
  }
  ...
  return (
    <section className="card">
      ...
      {state.status === "ok" && (
        <p>
          {state.amount} USDC <button onClick={refresh}>Refresh</button>
        </p>
      )}
      ...
      {state.status === "error" && (
        <p>
          {state.message} <button onClick={refresh}>Retry</button>
        </p>
      )}
